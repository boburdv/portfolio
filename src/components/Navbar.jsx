          ...
          {navbarLinks.map(({ text, path }, index) => {
            return (
              <li className="w-full" key={index}>
                <Link
                  className={`${buttonVariants({ variant: "link" })} ${pathname === path ? "underline" : ""} w-full`}
                  href={path}
                >
                  {text}
                </Link>
      ...
      <ul className="flex">
        {navbarLinks.map(({ text, path }, index) => {
          return (
            <li key={index}>
              <Link
                className={`${buttonVariants({ variant: "link" })} ${pathname === path ? "underline" : ""}`}
                href={path}
              >
                {text}
              </Link>
...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <Button onClick={() => setOpen((prev) => !prev)} className="sm:hidden" variant="outline" size="icon">
